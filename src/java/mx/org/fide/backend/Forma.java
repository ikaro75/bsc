...
public class Forma extends Consulta {
    ...
    /**
     * Recupera de la base de datos la definición de la forma a partir de la
     * llave primaria y carga los permisos asociados al perfil que invoca el
     * método. En caso de que la clave primaria de la forma sea negativa se
     * asignan datos reservados a los atributos del objeto puesto que así se
     * indica recuperación de datos de tablas de sistema. Las consultas
     * negativas para alimentar el diccionario de datos son establecidas en la
     * tabla be_consulta_forma
     *
        ...
        try {
            ...
            while (rsReportes.next()) {
                reporte = new Reporte(
                        ...
                        .replace("%area", "'".concat(super.getUsuario().getArea().toString()).concat("'"))
     ...
     */
    public ArrayList<Nota> getNotas(String claveRegistro) throws Fallo {
        ...
        try {
            ...
            s = "SELECT be_empleado.nombre + ' ' + be_empleado.apellido_paterno + ' ' + be_empleado.apellido_materno as nombre, be_empleado.foto, be_nota_forma.* "
                    + "FROM be_nota_forma, be_empleado WHERE be_nota_forma.clave_empleado=be_empleado.clave_empleado AND clave_forma=".concat(Integer.toString(this.claveForma)).concat(" AND clave_registro='").concat(claveRegistro.toString()).concat("'");
            rsNotas = oDb.getRs(s);
            ...
            while (rsNotas.next()) {
                nota = new Nota(
                        ...
                        rsNotas.getString("clave_registro"),
                        rsNotas.getString("clave_empleado"),
    ...
    public String importa(Integer aplicacion, String prefijoTabla, Usuario usuario) throws Fallo {
        ...
        try {
            ...
            if (cx.getDbType() == mx.org.fide.modelo.Conexion.DbType.MSSQL) {
                s = "SELECT DISTINCT so.name as table_name, sc.name as column_name FROM SYSOBJECTS so, syscolumns sc WHERE so.XTYPE='u' and so.id=sc.id and sc.colid=1 and so.name LIKE '".concat(prefijoTabla).concat("'");
