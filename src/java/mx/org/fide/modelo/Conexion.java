...
public class Conexion {
     ...
     * @throws Fallo    si ocurre algún error relacionado a la base de datos
     */
    public String getLogin(String sUser, String sPw) throws Fallo {
        ...
        s = "SELECT e.personal FROM personal e, be_perfil_aplicacion pa "
                + "WHERE e.clave_perfil=pa.clave_perfil AND "
                + "e.email='" + sUser.replace("'", "''") + "' AND "
                + "e.contrasena='" + sPw.replace("'", "''") + "' AND "
                + "pa.activo=1";
        ...
        try {
            ...
            if (rs.next()) {
                sUsuario = rs.getString("personal");
            }
            rs.close();
        } catch (Exception e) {
            // Fallo en algun momento.
