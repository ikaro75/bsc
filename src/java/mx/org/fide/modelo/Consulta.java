...
public class Consulta {
    ...
    public void setCampos() throws Fallo {
        ...
        try {
            ...
            for (int i = 0; i < oRs.getMetaData().getColumnCount(); i++) {
                ...
                if (oDb.getServer().equals(this.usuario.getCx().getServer()) && oDb.getDb().equals(this.usuario.getCx().getDb())) {
                    rsFieldDictionary = oDb.getRs(qryCampos.toString());
                } else {
                    oDbFields = new Conexion(this.usuario.getCx().getServer(), this.usuario.getCx().getDb(), this.usuario.getCx().getUser(), this.usuario.getCx().getPw(), this.usuario.getCx().getDbType());
                    rsFieldDictionary = oDbFields.getRs(qryCampos.toString());
                }
    ...
    public String setSQL(Integer claveForma, String tipoConsulta, String pk, String w) throws Fallo {
        ...
        try {
            ...
            if (w.equals("")) {
                if (this.claveTipoGrid == 2 && this.tipoConsulta.equals("select") && !this.sql.toLowerCase().contains("union")) {
            ...
            if (this.claveTipoGrid == 2 && this.tipoConsulta.equals("select") && !this.sql.toLowerCase().contains("union")) {
                ...
                //El desarrollador tiene la responsabilidad de proporcionar el campo _hijos con el número de hijos dentro de la consulta con unions
            }
    ...
    public String update(boolean ejecuta) throws Fallo {
        ...
        Campo llavePrimariaCT = (Campo) this.campos.get(this.llavePrimaria.toLowerCase());

        if (llavePrimariaCT.tipoDato.toLowerCase().equals("text")
                || llavePrimariaCT.tipoDato.toLowerCase().equals("varchar")
                || llavePrimariaCT.tipoDato.toLowerCase().equals("char")
                || llavePrimariaCT.tipoDato.toLowerCase().equals("date")
                || llavePrimariaCT.tipoDato.toLowerCase().equals("datetime")
                || llavePrimariaCT.tipoDato.toLowerCase().equals("smalldatetime")) {
            s.append(" WHERE ").append(this.llavePrimaria).append("='").append(this.pk).append("'");
        } else {
            ...
            if (oRs.next()) {
                ...
                while (it.hasNext()) {
                    ...
                    if (campo.valorOriginal == null && (campo.valor != null && !campo.valor.equals(""))) {
                        ...
                        if ((campo.tipoDato.toLowerCase().equals("text") || campo.tipoDato.toLowerCase().equals("varchar") || campo.tipoDato.toLowerCase().equals("char")) && !campo.valor.equals(campo.valorOriginal)) {
                            sonRealmenteValoresDiferentes = true;
                            ...
                            if (campo.valor.equals("%ahora")) {
                                sonRealmenteValoresDiferentes = true;
                            } else if (!new java.util.Date(campo.valor.split("/")[2].concat("/").concat(campo.valor.split("/")[1]).concat("/").concat(campo.valor.split("/")[0])).equals(new java.util.Date(campo.valorOriginal.substring(0, 10).replaceAll("-", "/")))) {
                                sonRealmenteValoresDiferentes = true;
                            }
    ...
    public Consulta(int claveForma, String tipoConsulta, String pk, String w, String valoresDeReemplazo, Usuario usuario, Integer registros, Integer pagina, String sidx, String sord) throws Fallo {
        ...
        try {
            ...
            if (this.noParsear) {
                ...
                Integer parentesisAbiertos = 0;
                Integer cierreParentesis = 0;
                Boolean estoyEnElWith = true;
                Boolean estoyEnElSelect = false;
                Boolean estoyEnElFrom = false;
                Boolean estoyEnElWhere = false;
                Boolean estoyEnElGroupBy = false;
                Boolean estoyEnElHavingBy = false;
                
              /* 1. La cadena empieza con un with ?
                 si: ahora busco que no haya parentesis abiertos y busco un select
                 no: la cadena empieza con select ?
                 no
                        
                
                for (int i = 0; i < s.length(); i++) {
                    //Busco donde acaba el with
                    if (s.substring(i, i+4).toLowerCase().startsWith("with")) {
                                with = s.substring(0, 3);
                                i = 3;
                            } else {
                                estoyEnElWith = false;
                                estoyEnElSelect = true;
                                
                        } else {
                            with += s.substring(i, i + 1);
                    ...
                    while (estoyEnElSelect) {
                        if (estoyEnElSelect && i == 0) {
                    ...
                    for (int i = 0; i < aFields.length; i++) {
                        ...
                        if (aFields[i].toLowerCase().startsWith("with")) {
                            ...
                            for (int j = 0; j < aWhere.length; j++) {
                                ...
                                if (esDelWhere) {
                                    where = where.concat(aWhere[j].concat(" "));
                                } else {
                                    from += ",".concat(aWhere[j]);
                                }
                            }

                            if (i < aFields.length && (aFields[i + 1].toLowerCase().startsWith("and") || aFields[i + 1].toLowerCase().startsWith("or"))) {
                                wherePendiente = true;
                            }
    ...
    public Consulta(int claveForma, String tipoConsulta, String pk, String w, Usuario usuario, Integer registros, Integer pagina, String sidx, String sord, Integer nodo) throws Fallo {
        ...
        try {
            ...
            if (oDb.getDbType() == Conexion.DbType.MYSQL) {
                ...
                //Primer select, la regla 
                if (!this.sql.toLowerCase().contains("union")) {
                    s = "SELECT "
                            .concat(" * FROM (")
