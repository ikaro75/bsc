...
( function($) {
    $.fn.appgrid = function(opc){
        $.fn.appgrid.settings = {
            ...
            valoresReemplazo:"",
            tipoConsulta:"select",
            desactivaBusqueda:false,
            desactivaOndblClickRow: false,
            onSelectRow: ""
        };
        ...
        return this.each( function(){
            ...
            //Si as? es toma de su id el sufijo app + entidad principal + entidad foranea            
            $(this).html("<table width='100%' id='grid"+ suffix +
                ...
                "' tipoConsulta='" + $.fn.appgrid.options.tipoConsulta  +
                "' desactivaOndblClickRow='" + $.fn.appgrid.options.desactivaOndblClickRow +
                "'>" +
    ...
    $.fn.appgrid.getGridDefinition = function(){
        ...
        $.ajax(
        {
            url: $.fn.appgrid.options.xmlUrl + "?$cmd=grid&$cf=" + $.fn.appgrid.options.entidad.split('-')[0] + "&$ta=" + $.fn.appgrid.options.tipoConsulta + "&$dp=header&$w=" + $.fn.appgrid.options.wsParameters +"&$vr=" + encodeURIComponent($.fn.appgrid.options.valoresReemplazo),
    ...
    function processGridDefinition (data){
        ...
        if (error.length>0) {
            ...
            if (error.text().indexOf("No se pudo realizar la conexi?n TCP/IP al servidor")>0) {
            ...
            if (error.text().indexOf("Error al ejecutar la consulta")>-1 && $("#_cp_").val()=="1") {
                $.fn.appgrid.options.error+=error.text()+ ", haga click <a href='#' id='lnkEditQuery-1_8_" +  
                $(error.text()).find("pk").text()+ "' class='editLink'>aqui</a> para editarla";
               ...
               $("#loader_"+ $.fn.appgrid.options.app + "_" + $.fn.appgrid.options.entidad + "_" + $.fn.appgrid.options.pk /*+ $.fn.appgrid.options.datestamp*/).html("<br><br><br>"+
                ...
                $(".editLink").click(function(){
                    ...
                    nApp=this.id.split("_")[0];
                    nForma=this.id.split("_")[1];
        ...
        /* Inicia implementaci?n del grid */
        var nApp=$.fn.appgrid.options.app;
        ...
        if ($("#grid"+suffix).attr("requeriesFilter")==0 ) //Si no est? configurado para filtrar registros, trae el cuerpo
            xmlURL=$.fn.appgrid.options.xmlUrl + "?$cmd=grid&$cf="+ nEntidad + "&$ta=" + $.fn.appgrid.options.tipoConsulta + "&$dp=body&$w=" + $.fn.appgrid.options.wsParameters;
        else if ($("#lnkRemoveFilter_grid"+suffix).length==0)  //Si est? configurado y no tiene un filtro solo trae la cabecera
            xmlURL=$.fn.appgrid.options.xmlUrl + "?$cmd=grid&$cf="+ nEntidad + "&$ta=" + $.fn.appgrid.options.tipoConsulta + "&$dp=body&$w=" + $.fn.appgrid.options.wsParameters;
        else
            xmlURL=$.fn.appgrid.options.xmlUrl + "?$cmd=grid&$cf="+ nEntidad + "&$ta=" + $.fn.appgrid.options.tipoConsulta + "&$dp=body&$w=" + $.fn.appgrid.options.wsParameters;
        ...
        else if ($("#formGrid_"+ $.fn.appgrid.options.app + "_" + $.fn.appgrid.options.entidad).length>0)
           $("#formGrid_"+ $.fn.appgrid.options.app + "_" + $.fn.appgrid.options.entidad).attr("original_url",xmlURL)        
        else if ($("#grid_suscriptores").length>0)
            $("#grid_suscriptores").attr("original_url",xmlURL);
        ...
        $(xml).find("column_definition").children().each(function(){
         ...
         oGridConfig= {
            url:xmlURL + "&$vr=" + encodeURIComponent($.fn.appgrid.options.valoresReemplazo),
            datatype: "xml",
          ...
          ondblClickRow: function(rowid) {
                    ...
                    if ($(this).attr("desactivaOndblClickRow")=="true") {
                        return;
                    }
            ...
            gridComplete:function(){
              ...
              //Cierra cualquier dialogo de espera
              $("#divwait").dialog( "close" );
            }
        };
        
        //Se programa el evento on onSelectRow
        if ($.fn.appgrid.options.onSelectRow!="") {
              oGridConfig.onSelectRow= $.fn.appgrid.options.onSelectRow;
        }
        ...
        .navButtonAdd('#grid'+ suffix+'_toppager',{
            ...
            cursor: "pointer"
                ...
                onClickButton:  function() {
                    ...
                    window.open(sUrl.replace("cmd=grid","cmd=download"), "_blank");
                    return false;            
                },
                position: "last",
                title:"Descargar catalogo en formato XLS",
                cursor: "pointer"
       });
        ...
        //Aparece un bot?n para editar la forma si se cuenta con el perfil de admnistrador
        if ($("#_cp_").val()=="1") {
            oGrid.navGrid('#grid'+ suffix+'_toppager',{
                edit:false,
                add:false,
        ...
        //Establece la funci?n para la liga lnkRemoveFilter_grid_ que remueve el filtro del grid
        if ($.fn.appgrid.options.wsParameters!="" && $.fn.appgrid.options.showFilterLink) {
            $("#lnkRemoveFilter_grid" + suffix).click(function() {
                ...
                if ($("#grid"+suffix).attr("requeriesFilter")=="1")
                    ...
                    $(sGridId).jqGrid('setGridParam',{
                        url:"control?$cmd=grid&$cf=" + nForma + "&$ta=" + $(sGridId).attr("tipoConsulta") + "&$dp=body"
                    }).trigger("reloadGrid")
            });
        }
        ...
        //Verifica si el grid est? en una cola
    ...
    /* Finaliza implementaci?n de grid */
