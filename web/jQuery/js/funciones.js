...
function check_date(field){
    ...
    /* Error-message if err != 0 */
        ...
        alert("Fecha no v?lida, verifique");
        DateField.value="";
        DateField.select();
        DateField.focus();
        return false;
...
function check_number(field,nRangoMenor,nRangoMayor) {
    ...
    if (isNaN(numberField)&&field.value!=='') {
        alert('Valor inv�lido, se debe indicar un n�mero, verifique.');
        field.value='';	
        field.select();
        field.focus();
        return false;
    ...
    else
    {
        ...
        else 
        {
            if (numberField>nRangoMayor) {
                alert('El n�mero indicado es mayor a ' + nRangoMayor +', verifique');
                field.select();
                field.focus();	
                return false;
            }
                ...
                if (numberField<nRangoMenor) {
                    alert('El n�mero indicado es menor a ' + nRangoMenor +', verifique');
                    field.select();
                    field.focus();
                    return false
                }
...
function right(e) {
    if (navigator.appName == 'Netscape' && (e.which == 3 || e.which == 2))
        ...
        alert("FIDE. Coordinaci�n de Inform�tica (c)");
        return false;
    }
    return true;
}
...
function setXMLInSelect3(sSelect,cf,ta,pk,w) {
    ...
    if (oSelect!=null) {
        ...
        for (i=0;i<x.length;i++) {
            ...
            var display=x[i].childNodes[3].childNodes[0].nodeValue.replace(/&aacute;/g,"�").replace(/&eacute;/g,"�").replace(/&iacute;/g,"�").replace(/&oacute;/g,"�").replace(/&uacute;/g,"�").replace(/&ntilde;/g,"�");
...
function setXMLInSelect4(sSelect,cf,ta,pk,w) {
    ...
    if (oSelect.length>0) {
        ...
        alert("No se encontr� el control indicado");
    }      
}
...
function emailCheck (emailStr) {
    ...
    if (domArr[domArr.length-1].length<2 || domArr[domArr.length-1].length>3) { 
        alert("La direcci�n debe tener 3 letras si es .'com' o 2 si en de alg�n pa�s.")
        return false
    }
