                ...
                $.ajax(
                        {
                            ...
                            success: function (data) {
                                ...
                                if ($.fn.form.options.modo == "update" && sPermiso.indexOf("3") > -1) {
                                    ...
                                    $("#formTab_" + formSuffix).html(sHTML.replace('<!-- InicioOpciones -->', '<div><input type="button" class="formButton" id="opciones_' + formSuffix + '" value="Opciones" style="top: -3px;"/><input type="button" class="formButton" id="btnMenuOpciones_' + formSuffix + '" value="" /></div><ul>'));
                                    sHTML = $("#formTab_" + formSuffix).html();
                                    $("#formTab_" + formSuffix).html(sHTML.replace('<!-- Eliminar -->', '<li><a id="btnEliminar_' + formSuffix + '" class="btnEliminar_' + formSuffix + '" >Eliminar</a></li>'));
                                }
                                ...
                                //Se extrae posibles escenarios que se podrán disparar al guardar la forma, 
                                ...
                                $(".btnEliminar_" + formSuffix).click(function () {
                                    var nodoEliminado = $("#formTab_" + formSuffix).attr("updateControl");
                                    if (!confirm("\xBFEst\u00e1 seguro que desea eliminar el registro? No es posible deshacer esta acci\u00f3n."))
                                        return false;

                                    $.ajax(
                                            {
                                                url: "control?$cmd=register&$ta=delete&$cf=" + $.fn.form.options.forma + "&$pk=" + $.fn.form.options.pk,
                                                dataType: "text",
                                                success: function (data) {
                                                    //Borra el nodo del ?rbol
                                                    $("#tvSuscriptores").jstree("delete_node", "#" + nodoEliminado);
                                                },
                                                error: function (xhr, err) {
                                                    if (xhr.responseText.indexOf("Iniciar sesi&oacute;n") > -1) {
                                                        alert("Su sesi?n ha expirado, por seguridad es necesario volverse a registrar");
                                ...
                                //Se asigna evento al bot?n de guardar
                                $("#btnGuardar_" + formSuffix).button().click(function () {
                                    ...
                                    if ($("#formTab_" + formSuffix).attr("modo") != "lookup") {
                                        ...
                                        if (sData == "") {
                                            ...
                                            $(oGridHeader[0]).append("<a href='#' id='lnkRemoveFilter_grid_" + nAplicacion + "_" + nForma + "_" + sDateStamp + "'>(Quitar filtro [" + sFiltro + "])</a>");
                                            ...
                                            else if ($("#formGrid_" + nAplicacion + "_" + nForma).length > 0)
                                                var previousUrl = $("#formGrid_" + nAplicacion + "_" + nForma).attr("original_url");
                                            else 
                                                var previousUrl = $("#grid_suscriptores").attr("original_url");
                                ...
                                /*for (i=0; i<$(".queued_grids").length;i++) {
                                 ...
                                 //para establecer posible relacion de forma y grid
                                ...
                                //Funci?n para editar diccionario desde la liga del alias
                                ...
                                function processXml(data) {
                                    ...
                                    var sResultado = $($(xmlResult).find("pk")[0]).text();
                                    ...
                                    if (sControl == "") {
                                        ...
                                        if (sControl == "avatar") {
                                            ...
                                            if (oControl.length > 0) {
                                                if (oControl[0].className.indexOf("jstree") > -1) {
                                                    //oControl.jstree("destroy");
                                                    //oControl.treeMenu.getTreeDefinition(oControl);
                                                    //Se est? anexando a un sector
                                                    //Si se est? insertando 
                                                    if ($.fn.form.options.modo == "insert") {
                                                        if (oControl[0].id.indexOf("sector") == 0) {
                                                            if ($("#_cache_").val() == "sector") {
                                                                newNode = {id: "sector_" + sResultado + "-0", data: $("#taxonomia").val(), icon: "/directorio/img/folder1.png", attr: {id: "sector_" + sResultado + "-0", rel: "sector"}};
                                                                $("#tvSuscriptores").jstree("create_node", "#" + sControl, 'inside', newNode, false, false);
                                                                //Hace falta insertar el evento al nodo
                                                                $("#sector_" + sResultado + "-0").attr("evento", "clave_taxonomia in (%clave_taxonomia)");
                                                                $("#_cache_").val("");
                                                            } else if ($("#_cache_").val() == "entidad") {
                                                                newNode = {id: "entidad_" + sResultado + "-0", data: $("#taxonomia").val(), icon: "/directorio/img/usuarios1.png)", attr: {id: "entidad_" + sResultado + "-0", rel: "entidad"}};
                                                                $("#tvSuscriptores").jstree("create_node", "#" + sControl, 'inside', newNode, false, false);
                                                                //Se hace insert de la relaci?n entre sector y entidad
                                                                postConfig = "$cf=647&$ta=insert&$pk=0&$ca=142" +
                                                                        "&clave_sector=" + sControl.split("_")[1].split("-")[0] +
                                                                        "&clave_entidad=" + sResultado;
                                                                $.post("control?$cmd=register&$ta=insert", postConfig);

                                                                $("#_cache_").val("");
                                                            }
                                                        } else if (oControl[0].id.indexOf("entidad") == 0) {
                                                            newNode = {id: "entidad_" + sResultado + "-0", data: $("#entidad").val(), icon: "/directorio/img/usuarios1.png)", attr: {id: "entidad_" + sResultado + "-0", rel: "entidad"}};
                                                            $("#tvSuscriptores").jstree("create_node", "#" + sControl, 'inside', newNode, false, false);
                                                        }
                                                    } else if ($.fn.form.options.modo == "update") {
                                                        if (oControl[0].id.indexOf("sector") == 0) {
                                                            $("#tvSuscriptores").jstree('rename_node', '#' + sControl, $("#taxonomia").val());
                                                        } else if (oControl[0].id.indexOf("entidad") == 0) {
                                                            $("#tvSuscriptores").jstree('rename_node', '#' + sControl, $("#taxonomia").val());
                                                        }
                                                ...
                                                } else if (sControl == "portletContacto") {
            ...
            $.fn.form.handleForm = function (xml) {
                ...
                oCampos.each(function () {
                    ...
                    if (nFormaForanea != undefined) {
                        ...
                        sRenglon += 'id="' + oCampo[0].nodeName + (($.fn.form.options.modo == "lookup" && bBusquedaPorRango) ? "$inicial" : "") + '" name="' + oCampo[0].nodeName + (($.fn.form.options.modo == "lookup" && bBusquedaPorRango) ? "$inicial" : "") + '" >';
                        ...
                        if (oCampo.find('tipo_control').text() == "textarea" || sTipoCampo == "text") {
                            ...
                            sRenglon += '<td class="etiqueta_forma_control1">' +
                                    '<input tipo_dato="' + sTipoCampo + '" id="' + oCampo[0].nodeName + (($.fn.form.options.modo == "lookup" && bBusquedaPorRango) ? "$inicial" : "") + '" name="' + oCampo[0].nodeName + (($.fn.form.options.modo == "lookup" && bBusquedaPorRango) ? "$inicial" : "") + '" ' +
                                    'tabindex="' + tabIndex + '" ';
                ...
                for (i = 0; i < aRows.length; i++) {
                    //Si se trata de un textarea, este ocupa todo el renglón
                    if (aRows[i].indexOf("textarea") > -1) {
                        sTr = "<tr >" + aRows[i].replace(/class="etiqueta_forma_control1"/g, ' style="width:40%" class="etiqueta_forma_control' + nCols + '" colspan="4"').replace(/etiqueta_forma1/g, "etiqueta_forma" + nCols).replace(/inputWidgeted1/g, "inputWidgeted" + nCols) + "</tr>";
                        sForm += "<tr >" + aRows[i].replace(/class="etiqueta_forma_control1"/g, ' style="width:40%" class="etiqueta_forma_control' + nCols + '" colspan="4"').replace(/etiqueta_forma1/g, "etiqueta_forma" + nCols).replace(/inputWidgeted1/g, "inputWidgeted" + nCols) + "</tr>";
                        abreTr = true;
                        sTr = "";
                    } else if (aRows[i] != "") {    
                        sTr += ((abreTr) ? "<tr>" : "<td style='width: 1%;'>&nbsp;</td>") + aRows[i].replace(/etiqueta_forma_control1/g, "etiqueta_forma_control" + nCols).replace(/etiqueta_forma1/g, "etiqueta_forma" + nCols).replace(/inputWidgeted1/g, "inputWidgeted" + nCols) + ((abreTr && nCols==1) ? "</tr>" : ((!abreTr && nCols>1)?"</tr>":""));
                        sForm += ((abreTr) ? "<tr>" : "<td style='width: 1%;'>&nbsp;</td>") + aRows[i].replace(/etiqueta_forma_control1/g, "etiqueta_forma_control" + nCols).replace(/etiqueta_forma1/g, "etiqueta_forma" + nCols).replace(/inputWidgeted1/g, "inputWidgeted" + nCols) + ((abreTr && nCols==1) ? "</tr>" : ((!abreTr && nCols>1)?"</tr>":""));
                        abreTr = (nCols==1?true:!abreTr);
                    } 
                }
